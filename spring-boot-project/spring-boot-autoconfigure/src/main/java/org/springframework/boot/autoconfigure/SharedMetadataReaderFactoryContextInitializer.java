 ...
 *
 * 这个初始化器给 Spring 容器添加一个 {@link CachingMetadataReaderFactoryPostProcessor}，它是一个 BeanFactoryPostProcessor,
 * 其作用是在 Spring 在包扫描前（即 {@link ConfigurationClassPostProcessor } 执行之前）给容器中添加一个
 * {@link SharedMetadataReaderFactoryBean}，它是一个 FactoryBean & ApplicationListener， 然后再给
 * {@link ConfigurationClassPostProcessor } metadataReaderFactory 属性添加一个属性值（实际创建赋值）
 *
 * todo huangran 确定这个属性值的作用
 *
 * @author Phillip Webb
 */
class SharedMetadataReaderFactoryContextInitializer
	...
	public void initialize(ConfigurableApplicationContext applicationContext) {
		// 向 Spring 容器中添加了一个 BeanFactoryBeanPostProcessor，并且必须在 ConfigurationClassPostProcessor 这个
		// 后置处理器之前执行
	...
	private static class CachingMetadataReaderFactoryPostProcessor
		...
		public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) throws BeansException {
			// 向 Spring 容器中注册一个 SharedMetadataReaderFactoryBean (FactoryBean & ApplicationListener)
			register(registry);
			// 给 ConfigurationClassPostProcessor 的 metadataReaderFactory 属性添加一个属性值
		...
		private void configureConfigurationClassPostProcessor(BeanDefinitionRegistry registry) {
			try {
				...
				// 给 ConfigurationClassPostProcessor 的 metadataReaderFactory 属性添加一个值
