...
class BeanDefinitionLoader {
	...
	private int load(Class<?> source) {
		...
		if (isEligible(source)) {
			// 将启动类的转换为 BeanDefinition 注册到 beanDefinitionMap 中
			// 后续 Spring 的 refresh 时就是把这个启动类作为配置类来解析，就可以解析到
			// 启动类上的 @SpringBootApplication 注解了
