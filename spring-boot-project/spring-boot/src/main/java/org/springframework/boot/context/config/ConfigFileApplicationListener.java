 ...
 * @since 1.0.0
 *  1) ApplicationEnvironmentPreparedEvent: 用于读取配置文件，并将配置文件中属性加载 environment 中.
 * SpringBoot 默认支持从 5 个位置 {@link #DEFAULT_SEARCH_LOCATIONS} 读取配置文件，并加载到 environment 中，并且
 * 优先级由低到高，高优先级的可以将低优先级的属性值给覆盖.
 *  2) ApplicationPreparedEvent: 调整 defaultProperties（如果存在） 的优先级至最低.
 *
 * Note: SpringBoot 默认文件在加载位置
 *   1) file:./config/
 *   2) file:./config/{@literal *}/
 *   3) file:./
 *   4) classpath:config/
 *   5) classpath:
 * 除此之外，还可以通过以下方式来加载配置文件
 * 	1) 通过这两个方法 {@link #setSearchLocations(String)} and {@link #setSearchNames(String)} 指定配置文件位置和名称.
 * 	2) 通过配置文件中 spring.config.location 或 spring.config.name 来指定加载额外的配置文件
 */
public class ConfigFileApplicationListener implements EnvironmentPostProcessor, SmartApplicationListener, Ordered {
	...
	public void onApplicationEvent(ApplicationEvent event) {
		...
		// 向 Spring 中添加一个后置处理器 PropertySourceOrderingPostProcessor，它也是一个 BeanFactoryPostProcessor
		// 其作用是如果环境中存在 defaultProperties，将其优先级降到最低（即将它放到 propertySources 列表的末尾）
	...
	private void onApplicationEnvironmentPreparedEvent(ApplicationEnvironmentPreparedEvent event) {
		/**
		 * CloudFoundryVcapEnvironmentPostProcessor: SpringBoot 无操作
		 * SpringApplicationJsonEnvironmentPostProcessor: 解析环境中的 json 属性成 PropertySource
		 * SystemEnvironmentPropertySourceEnvironmentPostProcessor: 转换 systemProperties 系统变量的类型
		 * DebugAgentEnvironmentPostProcessor: 无操作
		 * ConfigFileApplicationListener: 该监听器本身也实现了 EnvironmentPostProcessor 接口，完成对配置文件的加载
		 */
	...
	protected void addPropertySources(ConfigurableEnvironment environment, ResourceLoader resourceLoader) {
		// 添加配置文件中的随机数的 PropertySource
		RandomValuePropertySource.addToEnvironment(environment);
		// 完成对配置文件的加载
		// 从 META-INF/spring.factories 文件中读取出所有的配置加载器，用于加载应用中的配置文件
		// 具体的加载逻辑在 load 中
	...
	private class Loader {
		...
		Loader(ConfigurableEnvironment environment, ResourceLoader resourceLoader) {
			...
			/**
			 * 从 META-INF/spring.factories 文件中读取出所有的 PropertySourceLoader，主要用于加载配置文件
			 * 1) PropertiesPropertySourceLoader --> .perperties
			 * 2) YamlPropertySourceLoader  --> .yml
			 * 配置文件的加载器
			 */
		...
		/**
		 * 完成对配置文件中的属性加载工作
		 * Spring 默认支持从 {@link #DEFAULT_SEARCH_LOCATIONS} 这 6 个位置查找配置文件，然后加载到环境中
		 * todo huangran 配置文件加载逻辑，比较复杂
		 */
		void load() {
