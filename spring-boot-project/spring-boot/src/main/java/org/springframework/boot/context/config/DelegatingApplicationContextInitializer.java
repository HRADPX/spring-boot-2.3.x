 ...
 *
 * SpringBoot 委托这个初始化器将环境中变量名为 {@link #PROPERTY_NAME} 的属性值所表示的
 * ApplicationContextInitializer 实例化后，调用 {@link ApplicationContextInitializer#initialize}
 * 方法.
 * Note:
 * 1) 此时，配置文件已经被加载，所以可以在配置文件中配置这个变量，也可以在 spring.factories 中定义自己的初始化器.
 * 2) 这个 {@link #PROPERTY_NAME} 变量的属性值必须是 {@link ApplicationContextInitializer} 的实现类，多个用逗号分隔.
 *
 * @author Dave Syer
 * @author Phillip Webb
 * @since 1.0.0
