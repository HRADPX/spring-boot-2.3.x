 ...
 * </li>
 * </ul>
 * 这个监听器是对 SpringBoot 日志系统的设置
 * 1) ApplicationStartingEvent: 设置具体的日志实现类，如果要自定义日志系统，需要覆盖环境中的 {@link LoggingSystem#SYSTEM_PROPERTY}
 * 属性，默认情况下，SpringBoot 使用的是 LogbackLoggingSystem.
 * @see LoggingSystem#get(java.lang.ClassLoader)
 * Note: 由于 SpringBoot 设置日志系统的在 SpringBoot 启动时就开始，所以如果要覆盖默认的日志系统，不能在配置文件中设置
 * {@link LoggingSystem#SYSTEM_PROPERTY} 属性，而是要在这个监听器之前设置，例如添加一个监听器，并且优先级高于该监听器即可。
 * 2) ApplicationEnvironmentPreparedEvent: 初始化日志系统，具体操作未知。
 * 3) ApplicationPreparedEvent: 注册日志具体实现、日志文件、日志系统到 Spring 容器中。
 *
 * logging.config: 配置日志
 * logging.level.*: 日志输出级别
 * logging.file.path: 输入日志文件的路径
...
public class LoggingApplicationListener implements GenericApplicationListener {
	...
	public void onApplicationEvent(ApplicationEvent event) {
		if (event instanceof ApplicationStartingEvent) {
			/**
			 * 选择 SpringBoot 具体的日志实现，默认使用的是 LogbackLoggingSystem
			 * @see LoggingSystem#get(java.lang.ClassLoader)
			 */
			...
			/**
			 * 注册日志具体实现、日志文件、日志组对象到 Spring 容器中
			 */
