 ...
 *
 * 用于解析 JSON，{@code spring.application.json} 和 {@code SPRING_APPLICATION_JSON} 并将解析的结果转换为
 * PropertySource 添加到 propertySources 中.
 *
 * Note: 此时还没有加载加置文件 .properties 和 .yml，所以解析的不是配置文件的中 json 属性
 *
 * @author Dave Syer
 * @author Phillip Webb
...
public class SpringApplicationJsonEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
	...
	public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
		// 遍历环境中的 propertySources，判断是否有 json 属性，如果有将其解析成为 PropertySource
		// {@code SPRING_APPLICATION_JSON_PROPERTY} 和 {@code SPRING_APPLICATION_JSON_ENVIRONMENT_VARIABLE}
		// Note: 此时还没有加载加置文件 .properties 和 .yml，所以解析的不是配置文件的中 json 属性
