 ...
 *
 * 这个接口用于自动配置 servlet 3.0+，和 WebApplicationInitializer 不同，实现这个接口但没有实现 WebApplicationInitializer 它不会
 * 被 SpringServletContainerInitializer 识别，因此 Servlet 容器启动时不会自动引导这些类，它的生命周期是由 spring 管理。
 * @see WebApplicationInitializer
 *
 * 接口的作用是向 servletContext 中注册 servlet，filter，listener 等，该接口的生命周期是由 Spring 管理的。
 */
@FunctionalInterface
public interface ServletContextInitializer {
	 ...
	 * 对给的 servletContext 配置任何的 servlets、filters、listeners、context-params
	 */
