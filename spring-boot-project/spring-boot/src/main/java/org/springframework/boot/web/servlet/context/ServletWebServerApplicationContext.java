...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	protected void onRefresh() {
		super.onRefresh();
		try {
			// 该接口覆盖了 AbstractApplicationContext 中的方法，在 spring 创建普通的 bean 之前
			// 调用这个方法来获取嵌入式的 Servlet 容器工厂，并通过调用工厂的 getWebServer 方法来创建
			// 启动 Tomcat，并将 DispatcherServlet 添加到容器中
			/** SpringMVC 的组件是注册是通过 WebMvcAutoConfiguration 这个 config class 来注册的. */
			createWebServer();
		}
		catch (Throwable ex) {
	...
	private void createWebServer() {
		...
		if (webServer == null && servletContext == null) {
			// 首先获取嵌入式 Servlet 容器工厂，这里就是自动配置的
			...
			// 这里获取的是 WebServerFactory 工厂
			ServletWebServerFactory factory = getWebServerFactory();
			// 调用工厂类的 getWebServer 方法实例化 Web Server 容器(默认 Tomcat) 并启动
			// 这里 getSelfInitializer() 是获取 ServletContextInitializer
			// 它返回的是 this::selfInitialize，所以后续执行接口方法的时候，
			// 执行的代码是 selfInitialize() 方法的逻辑
